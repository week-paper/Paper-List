...
class OctConv(nn.Module):
    def __init__(self, ch_in, ch_out, kernel_size, padding=0, stride=1, alphas=(0.5, 0.5)):
        ...
        # FILTERS
        self.wHtoH = nn.Parameter(torch.randn(
            self.ch_out_hf, self.ch_in_hf, kernel_size, kernel_size))
        self.wHtoL = nn.Parameter(torch.randn(
            self.ch_out_lf, self.ch_in_hf, kernel_size, kernel_size))
        self.wLtoH = nn.Parameter(torch.randn(
            self.ch_out_hf, self.ch_in_lf, kernel_size, kernel_size))
        self.wLtoL = nn.Parameter(torch.randn(
            self.ch_out_lf, self.ch_in_lf, kernel_size, kernel_size))
    ...
    def forward(self, input):
        ...
        if self.alpha_in == 0:
            ...
            hf_input = input[:, :self.ch_in_hf *
                             4, ...].reshape(-1, self.ch_in_hf, fmap_size * 2, fmap_size * 2)
        ...
        if self.alpha_in < 1:
            ...
            if self.ch_out_lf > 0:
                HtoL = F.conv2d(F.avg_pool2d(hf_input, 2),
                                self.wHtoL, padding=self.padding)
        if self.alpha_in > 0:
        ...
        if 0 < self.alpha_out < 1:
            ...
            hf_output = hf_output.reshape(-1, 4 *
                                          self.ch_out_hf, fmap_size, fmap_size)
            # cat over channel dim
